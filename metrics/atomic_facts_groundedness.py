...
def calculate_metrics(y_true, y_pred, y_true_binary=None, y_scores=None):
    ...
    if y_true_binary is not None and y_scores is not None and len(y_true_binary) > 0:
        try:
            gt = y_true_binary
            pred_score = y_scores
            
            auroc = roc_auc_score(gt, pred_score)
            
            # Treat 0 as positive by inverting labels and scores
            gt_inv = [1 - g for g in gt]
            pred_score_inv = [1.0 - p for p in pred_score]
            auprc_for_zero = average_precision_score(gt_inv, pred_score_inv)
            
            spearman_corr, _ = spearmanr(gt, pred_score)

            metrics["auroc"] = auroc
            metrics["auprc"] = auprc_for_zero
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate groundedness with atomic facts")
    parser.add_argument("--jsonl_file", type=str, default="Qwen-VL-Series-Finetune/groundedness/qwen/split/gpt5_sampled_evaluated_facts_210.jsonl")
    parser.add_argument("--csv_file", type=str, default="Qwen-VL-Series-Finetune/groundedness/qwen/split/annotations_merged.csv")
    args = parser.parse_args()

    base_dir = os.path.dirname(args.jsonl_file)
    ...
    with open(jsonl_file, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            evaluated_facts_str = item.get('evaluated_facts', '')
            
            # Clean up the markdown in evaluated_facts string if it exists
            clean_str = evaluated_facts_str.strip()
            ...
            try:
                ...
                for fact in facts:
                    label = fact.get('label', '').strip().lower()
                    if label == 'yes':
                        yes_count += 1
                    elif label == 'no':
                        no_count += 1
                        
                ratio_score = yes_count / (yes_count + no_count) if (yes_count + no_count) > 0 else 0.0
                is_grounded = 'yes' if ratio_score >= 1.0 else 'no'
            except Exception as e:
                # Fallback to regex if JSON is malformed
                labels = re.findall(r'\"label\"\s*:\s*\"?(Yes|No|yes|no)\"?', evaluated_facts_str, re.IGNORECASE)
                if not labels:
                    print(f"Could not parse or find labels using regex for entry id {idx}")
                    continue
                else:
                    yes_count = 0
                    no_count = 0
            ...
            y_true_binary.append(1 if majority_vote == 'yes' else 0)
            y_scores.append(ratio_score)
            ...
            output_results.append({
                ...
                "evaluated_facts": evaluated_facts_str,
                "u1": u1,
                "u2": u2,
                "u3": u3
            })
    ...
    metrics_majority = calculate_metrics(y_true_majority, y_pred, y_true_binary, y_scores)
    ...
    out_basename = file_basename.replace('.jsonl', '_metrics_detailed.jsonl')
    out_file = os.path.join(base_dir, out_basename)
