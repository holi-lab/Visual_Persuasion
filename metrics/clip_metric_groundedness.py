...
def main():
    parser = argparse.ArgumentParser(description="Calculate metrics for a given JSONL file against annotator ground truth.")
    parser.add_argument("--jsonl_file", type=str, default="Qwen-VL-Series-Finetune/groundedness/qwen/split/clip_image_eval_results_210.jsonl", help="Path to the JSONL evaluation results file")
    parser.add_argument("--csv_file", type=str, default="Qwen-VL-Series-Finetune/groundedness/qwen/split/annotations_merged.csv", help="Path to the annotator CSV file")
    ...
    with open(jsonl_file, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            # Determine prediction based on file type thresholds
            if file_basename.startswith('clip_image_eval_results'):
                sim = float(item.get('clip_similarity', 0.0))
                pred_label = 'yes' if sim > 0.3 else 'no'
            elif file_basename.startswith('clip_eval_results'):
                sim = float(item.get('clip_similarity', 0.0))
                pred_label = 'yes' if sim > 0.6 else 'no'
            else:
                # Handle old JSONL type with 'evaluated_reason'
